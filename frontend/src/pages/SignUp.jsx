...
try {
 ...
 <form className='w-full min-h-[430px] sm:min-h-[520px] max-h-[calc(100dvh-48px)] max-w-[500px] overflow-y-auto bg-[#00000062] backdrop-blur shadow-lg shadow-black flex flex-col items-center justify-center gap-[14px] sm:gap-[20px] px-[20px] sm:px-[24px] py-[28px] sm:py-[36px]' onSubmit={handleSignUp}>
<h1 className='text-white text-[24px] sm:text-[30px] font-semibold mb-[18px] sm:mb-[30px] text-center leading-tight'>Register to <span className='text-blue-400'>Virtual Assistant</span></h1>
<input type="text" placeholder='Enter your Name' className='w-full h-[52px] sm:h-[60px] outline-none border-2 border-white bg-transparent  text-white placeholder-gray-300 px-[20px] py-[10px] rounded-full text-[16px] sm:text-[18px]' required onChange={(e)=>setName(e.target.value)} value={name}/>
...
<button className='min-w-[140px] sm:min-w-[150px] h-[52px] sm:h-[60px] mt-[18px] sm:mt-[30px] text-black font-semibold  bg-white rounded-full text-[17px] sm:text-[19px] ' disabled={loading}>{loading?"Loading...":"Sign Up"}</button>

<p className='text-[white] text-[15px] sm:text-[18px] cursor-pointer text-center' onClick={()=>navigate("/signin")}>Already have an account ? <span className='text-blue-400'>Sign In</span></p>
 </form>
    </div>
  )
}
